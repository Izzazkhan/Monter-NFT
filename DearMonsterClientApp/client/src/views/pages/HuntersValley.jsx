		...
		if (true) {
			...
			DMSTokenContract.methods.balanceOf(accounts[0]).call().then(async function (balance) {
				...
				for (let i = 0; i < _attributes.length; i++) {
					for (let j = 0; j < _attributes[i].length; j++) {
						let obj = {}
						obj['name'] = _attributes[i]['name']
						obj['level'] = _attributes[i]['level']
						obj['OwnerID'] = _attributes[i]['owner']
						arr.push(obj)
				...
				// 		}
				// 	}
				// 	const response = await axios.post(Monster, params, config)
